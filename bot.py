...
async def add_schedule_command(message: types.Message):
    # Разбор аргументов команды и добавление урока в базу данных.
    try:
        args = message.get_args().split(" : ")
        if len(args) != 3:
            raise ValueError("Формат должен быть: Предмет : Преподаватель : Кабинет")
        subject, teachers, classroom = args
        add_schedule_entry(subject, teachers, classroom)  
        await message.reply("Урок успешно добавлен в расписание.")
